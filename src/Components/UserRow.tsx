...
import { openEditModal, openConfirmModal, AddUserInStage, updateFormData } from '../Store/ModalSlice';
...
const UserRow: React.FC<UserRowProps> = ({ usuario }) => {
    const dispatch = useDispatch();

    const handleEdit = () => {
        dispatch(AddUserInStage({ userKey: usuario.key }));
        dispatch(updateFormData(usuario));
    ...
    return (
        ...
        <tr key={usuario.key} className="border-b dark:border-gray-700">
            ...
            <td className="px-4 py-3 flex items-center justify-end">
                <button onClick={handleEdit} className="flex w-full items-center py-2 group duration-300">
                ...
                <button onClick={handleDelete} className="flex w-full items-center py-2 text-red-500 group duration-300">
