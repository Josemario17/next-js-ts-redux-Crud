...
import { closeEditModal } from '@/Store/ModalSlice';
import AddButton from './AddButton';
import { RootState } from '@/Store/store';

export default function ModalEdit() {
  const dispatch = useDispatch();
  const isModalOpen = useSelector((state: RootState) => state.modal.editModal);
        ...
        <div className="relative p-4 mx-auto w-full max-w-2xl max-h-full">
          <div className="relative p-4 bg-white rounded-lg dark:bg-gray-800 sm:p-5 shadow-2xl border border-solid border-gray-600 ">
            <div className="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
              ...
              <button onClick={() => dispatch(closeEditModal())} type="button" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <form action="#">
              ...
              <AddButton key={0} text='Atualizar Dados' typeButton='submit' handleClick={() => {}} />
            </form>
          </div>
        </div>
      </div>
