...
export default function Home() {
  ...
  const [ModalEditState, setModalEdit] = useState<boolean>(false);
  const [ModalConfirmState, setModalConfirm] = useState<boolean>(false);
          ...
          <div className="bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
              ...
              <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                <AddButton text="Adicionar Usuario" typeButton="button" handleClick={() => { setModalCreateState(!ModalCreateState) }}></AddButton>
              </div>
            </div>
            <TableList key={0} DeleteModalState={setModalConfirm} EditModalState={setModalEdit}></TableList>
          </div>
        </div>
      </section>

      {ModalCreateState && <ModalCreate ModalState={setModalCreateState}></ModalCreate>}
      {ModalEditState && <ModalEdit ModalState={setModalEdit}></ModalEdit> }
      { ModalConfirmState && <ModalConfirm ModalState={setModalConfirm}></ModalConfirm> }
    </div>
  );
}